(function () {
  ...
  function registerInitCallback(callback) {
    _initCallbacks ? _initCallbacks.push(callback) : callback();
  }
  ...
  function registerReadyCallback(callback) {
    _readyCallbacks ? _readyCallbacks.push(callback) : callback();
  }
  ...
  function _loadScripts(scriptNames, callback, context) {
    ...
    for (var i = 0, scriptCount = scriptNames.length; i < scriptCount; i++) {
